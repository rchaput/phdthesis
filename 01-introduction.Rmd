...
A potential risk, which is frequently mentioned, is the "Value Alignment Problem" (Dignum, Russell).
...
First, ethical stakes and the whole ethical knowledge that is necessary for ethical decision-making, i.e., the ability to take decisions that integrate ethical considerations and would be deemed as ethically-aligned by humans, necessarily comes from humans.
...
Despite this, we still would like them to take actions that consider, as much as possible, ethical considerations, so as to improve our lives.
...
In order to make artificial agents capable to take morally good actions, rather than evil, we argue they need to receive ethical considerations, from those who possess them, i.e., humans.
...
2. Learn ethical behaviours, in situations, which are well-aligned with human values.
    ...
    3. The system must learn according to a well-defined specification of the desired behaviour, to avoid "gaming" or mis-alignment.
...
The table \@ref(tab:table-recap-intro) summarizes the relation between the objectives, hypotheses, research questions and contributions.


<!-- Recap Table -->
```{r table-recap-intro}
#| tab.cap: >
#|   Associations between objectives, design choices, research questions, and contributions.
...
Chapter \@ref(positioning) specifies our positioning, methodological approach, presents our architecture conceptually and briefly describes our application case.
...
We propose two ways to do this, the first one through logical rules and Beliefs-Desires-Intentions agents, while the second one uses argumentation graphs.
...
Finally, the agents learn to perform the actions chosen by the users in similar contexts; we particularly emphasize the contextual aspect of user preferences.
