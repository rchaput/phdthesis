...
In the previous chapter, we described 2 algorithms to learn behaviours with ethical considerations.
The logical next step is thus to guide the learning agents through a reward signal, which indicates the degree of their actions' correctness.
...
Would it be acceptable to take an action almost perfect, from a moral point of view, which leads to situations where it is difficult or infeasible to take future morally good actions?
...
This in line with our second research question: (ref:rq2)

Traditionally, most reinforcement learning algorithms use some sort of mathematical functions to compute the rewards, as we have done in the previous chapter.
This seems rather intuitive, since the reward function should output a real number.
...
Indeed, the reward function serves as an incentive, it describes the behaviour that we expect from the agent.
...
Symbolic reasoning has already been used to judge the behaviour of other agents', perhaps we can extend this judgment to the computation of rewards?
...
In this chapter, we propose 2 different ways of leveraging such symbolic methods for judgments.
First, we partially use the Ethicaa platform to create judging agents based on logic reasoning, beliefs, and Prolog-like rules.
...
We can see that the "Compute reward" function, which was previously within the environment, is now "agentified", by introducing *judging agents*.
These judging agents rely on an explicitly defined moral value and its associated moral rules, to determine the reward that should be sent to each learning agent.
Multiple moral values are considered, with one specific judging agent for each moral value: this means that multiple rewards are produced for each learning agent.
However, the learning algorithm expects a single, scalar reward.
Thus, the rewards from the different judging agents are aggregated before they are sent.

```{drawio judgments-abstract-architecture, src="figure/contribution2_judging.drawio", out.width = "100%"}
#| fig.cap: >
#|   The abstract architecture of the symbolic judgment. The environment and
...
The 2 propositions, logic-based and argumentation-based judging agents, are detailed below, as well as several experiments that demonstrate learning agents are able to correctly learn from such symbolic-based rewards.
...
::: {.remark}
These 2 works have been done in association with interns, Jérémy Duval, who worked on the first part, judging through Ethicaa agents [@duval2020 ; @chaput2021multi], and Benoît Alcaraz, who worked on the second part, judging through argumentation [@alcaraz2021].
Christopher Leturc, who was at the time an associate lecturer, brought his expertise on argumentation for this second internship.
Although these works are not solely my own, I was, in both cases, co-involved in the internships' supervision, and thus helped shape those ideas and the resulting models.
I also expanded their work after their internships ended. 
:::

## Reward through logic rules {#judgments-ethicaa}
...
To perform the symbolic judgment, we introduce new, specific judging agents that are based on Ethicaa agents, and which we then describe.
...
As mentioned previously, this contribution focuses on leveraging Ethicaa agents to judge the learning agents' actions, and determine an appropriate reward.
There are multiple advantages and reasons behind this idea.

* The combination of symbolic (top-down) judgments and neural (bottom-up) learning makes a hybrid approach, which combines advantages of both.
  * Neural learning have the ability to generalize over unexpected situations.
  * Symbolic reasoning offer a better intelligibility of the expected behaviour, and the possibility to integrate prior knowledge from domain experts.
...
* Agentifying the reward function, by introducing judging agents, allows the judging and learning agents to evolve independently, and paves the way to co-construction.
  * Judging agents' rules can be updated by human designers, and learning agents must adapt their behaviour to comply with the new rules. This leads to a perspective of human-centered AI with a human-in-the-loop schema.
...
* As mentioned in our general framework, intelligibility is important, especially to confirm whether the expected behaviour is aligned with our desired moral values.
  * The judgment process is implemented on explicit, symbolic moral values and rules, which improves the intelligibility.

* We obtain a richer feedback by combining the judgment of multiple judging agents, which each correspond to a single moral value.
  * This facilitates the implementation of one moral value, and makes it clearer.
  * It also opens up more complex interactions between different moral values, represented by agents, such as negotiation processes.
  * Finally, it offers a way to update rules by adding, removing, or replacing judging agents.

We begin by introducing our judging agents, which are based on Ethicaa agents, how do they work, and how do they produce rewards.

### Judging agents

To perform the judgment, we leverage the existing Ethicca agents [@cointe2016ethicalJudgmentAgents] that we have already mentioned in the State of the Art.
...
An Ethicaa agent may perform the judgment on itself to determine which action it should take, or on another agent to determine whether it agrees with the other agent's behaviour.
We propose to adapt this judgment of others to compute the reward functions for learning agents.
Thus, we simplify their model to retain only the components that we need for our approach.
Figure \@ref(fig:ethicaa-agent) represents such an agent, with an explicit base of moral rules (MR), and moral values (VS).

```{drawio ethicaa-agent, src="figure/ethicaa.drawio", cache=TRUE, page.index=0, out.width = "100%"}
#| fig.cap: >
#|   The simplified Ethicaa agents' architecture, adapted from [@cointe2016ethicalJudgmentAgents].
```

Our proposed simplified agents use their beliefs about the current situation ($\mathcal{B}$), the moral values (VS), the moral rules (MR), and the knowledge about actions (A) to produce a Moral Evaluation ($\texttt{ME}$) of the actions.
The Moral Evaluation returns a symbol, either $\texttt{moral}$, $\texttt{immoral}$ or $\texttt{neutral}$, from which we will compute the reward.
We describe how we leverage this for our proposed model in the next section.

### The LAJIMA proposed model

We now describe the Logic-based Agents for JudgIng Morally-embedded Actions (LAJIMA), which is represented in Figure \@ref(fig:judgments-logic-process-schema).
...
This contribution focuses on the reward function, we will therefore largely ignore the rest of the RL framework.
For all things necessary, we assume the same DecPOMDP framework as described in the previous chapter.
...
* The observations and actions are multi-dimensional and continuous.
* The RL agent expect a scalar reward signal.

These assumptions are sufficiently generic to support a large part of the existing RL algorithms, in addition to our own Q-SOM and Q-DSOM algorithms.
They mean that the judging agents will have to handle real vectors as inputs, and produce a real scalar as output.

At each step of the simulation, after the learnings agents took an action, the observations and selected actions are sent to the judging agents.
We recall, from the DecPOMDP framework \@ref(def:decpomdp), that an observation $\mathbf{o}$ is a vector $\in \mathbb{R}^k$, and an action $\mathbf{a}$ is a vector $\in \mathbb{R}^d$.
In this case, observations received by judging agents are the same as observations received by learning agents: in Ethicaa's vocabulary, this is deemed to be a *partially informed ethical judgment*, since the judging agents have some information about the judged agents, i.e., the observations they used to take a decision.
However, they do not know the full state of the environment, nor the exact process that learning agents used.

From these observations, judging agents generate beliefs ($\mathcal{B}$) about the current situation, simply by creating a belief for each component of the vector, with the same name as the component, and a parameter that corresponds to the component's value.
...
Let us imagine a learning agent with a "Household" building profile, which has an action range of `r format(py$household$action_limit, big.mark = ',', scientific = F)`W.
...
<!-- TODO: maybe we need to introduce time steps in the following definitions and formulas? In particular, because we have the observations at the previous step, and the observations at the current step, e.g., o_l^{t-1} and o_l^{t}? -->

Judging agents use the set of generated values, known moral values and associated moral rules to determine if each component of the action, or "action symbol" supports or defeats their moral value.
To do so, we define the Moral Evaluation mechanism of judging agent $j$ as a function $\texttt{ME}_j : \mathbb{B} \times \mathbb{R} \rightarrow \mathbb{V}$, where we define $\mathbb{V}$ as the set of possible valuations $\mathbb{V} = \left\{ moral, immoral, neutral \right\}$.
...
If the action component defeats the value, or one of its sub-values, the evaluation is said to be `immoral`.
Otherwise, if the action component supports the value, its evaluation is said to be `moral`.
A default `neutral` evaluation is assigned when the action component neither supports nor defeats the value.
...
Mathematically, we define $\texttt{Judgment}_j(l) = \left\{ \forall i \in [[1,d]] : \texttt{ME}_j(\texttt{beliefs}(\mathbf{o_l}), a_{l,i}) \right\}$, where $\mathbf{o_l}$ is the observation vector received by $l$, and $\mathbf{a_l}$ is the action vector chosen by $l$.
...
We thus define the feedback of a learning agent $l$ as the judgment of all judging agents: $\texttt{F}(l) = \left\{ \forall j \in \mathbb{J} : \texttt{Judgment}_j(l) \right\}$.
The feedback function $\texttt{F} : \mathbb{L} \rightarrow \left( \mathbb{V}^d \right)^{\left|\mathbb{J}\right|}$ thus returns a list of lists of valuations.
...
Assume the feedback received by $l$ is
\begin{align*}
  \texttt{F}(l) &= \left\{ \texttt{Judgment}_{j_1}(l), \texttt{Judgment}_{j_2}(l) \right \} \\
...
Finally, the reward function $\texttt{R}_l : \mathbb{S} \times \mathbb{A}_l \times \mathbb{S} \rightarrow \mathbb{R}$ must return a single, real number.
However, we have a set of valuations.
Additionally, as we have seen in the previous example, we may have conflicts between judgments, both within a single judge, and between judges.
...
Such methods, how they differ with ours, and what they entail, are discussed in Section \@ref(judgments-discussion).

First, the judgment of a single judging agent, i.e., a list of valuations, is transformed into a single number, by counting the number of $moral$, and dividing it by the sum of the number of $moral$ and $immoral$ valuations.
...
If an action only received $neutral$ valuations, we consider it was neither good nor bad, and we set the number to $0.5$ as a special case.
...
To solve the conflicts between judges, and produce a single reward for all judges, we set the reward $r_l$ to be the average of these judgment numbers produced by each judging agent.
...
%      \tcc{Compute beliefs over the observations}
      $\mathbf{B} \gets \texttt{beliefs}(\mathbf{o_l})$\;
%      \tcc{Compute judgment}
      $\texttt{Judgment}_j(l) \gets \emptyset$\;
      \ForEach{dimension $i \in [[1,d]]$}{
        $\texttt{Judgment}_j(l)_i \gets \texttt{ME}(\mathbf{B}, a_{l,i})$\;
      }
      $\texttt{F}(l)_j \gets \texttt{Judgment}_j(l)$\;
    }
    \tcc{Transform symbolic judgments into a single scalar reward}
    $\mathbf{f_l} \gets \emptyset$\;
    \ForEach{judgment $\texttt{Judgment}_j(l) \in \texttt{F}(l)_j$}{
      \If{at least one moral or immoral valuation in $\texttt{Judgment}_j(l)$}{
        $moral \gets \left| \texttt{moral} \in \texttt{Judgment}_j(l) \right| $\;
        $immoral \gets \left| \texttt{immoral} \in \texttt{Judgment}_j(l) \right| $\;
    ...
    $r_l \gets average\left( \left\{ \forall f_{l,j} \in \mathbf{f_l} \right\} \right)$\;
  }
  \Return{$\mathbf{r}$}
}
```

## Reward through argumentation {#judgments-argumentation}
...
* Note that it is easy for designers to structure the rules, as we have mentioned in the previous point. The attack relationship makes explicit the impact of an argument on another.
...
In order to introduce our model, based on argumentation decision frameworks, we briefly cover some necessary knowledge about argumentation.

Argumentation allow us to express knowledge as a set of arguments, such as "There are clouds today", "The weather is nice".
Arguments are arranged as nodes of a graph, where the links between nodes represent attack relations.
...
Multiple argumentation frameworks exist, e.g., arguments can also have a priority, or weight, attacks can be unidirectional or bidirectional, etc.
...
* $\Args$ is a non-empty set of arguments. An argument is represented by a name, and has an aliveness condition function.
* $\Att$ is a binary relation named the attack relation, defined on pairs of arguments.
...
Indeed, this property means that we can compute the extension, and be guaranteed of its unicity, without worrying about having to implement in the judging agent a choice mechanism between possibles sets.
The *grounded* extension also has the advantage of being computed through a very efficient algorithm in $\mathcal{O}\left(\left|\Args\right| + \left|\Att\right|\right)$ time, where $\left|\Args\right|$ is the number of arguments in the whole graph, and $\left|\Att\right|$ is the number of attacks between arguments in $\Args$ [@nofal2021computing].
...
* $\forall j \in \JAgts \quad \epsilon_j : \LAgts \times AF \times \States \to \mathcal{P}(AF_j)$ is a function that filters the AFJD to return the sub-AFJD that judging agent $j$ uses to judge the learning agent $i$.
* $\forall j \in \JAgts \quad \Jj : \mathcal{P}(AF_j) \to \mathbb{R}$ is the judgment function, which returns the reward from the sub-AFJD.
* $\gagr : \mathbb{R}^{\left|\JAgts\right|} \to \mathbb{R}$ is the aggregation function for rewards.
:::
...
The $\epsilon_j$ function is used to filter the AFJD that is used by a judging agent, according to the current state of the world.
...
This allows comparing the learning agent to other agents, e.g., for the average consumption, which we cannot access if we only have the learning agent's observations $\mathbf{o}$.
This assumption can be lifted by changing the definition of $\epsilon_j$ to take $\mathbb{O}_l$ as domain ; however, this would limit the available arguments, and thus the judgment as a whole.
...
This pre-computation makes the argumentation graphs easier to design, since they can directly rely on these elements rather than performing the computation themselves, and avoids some overhead when 2 different graphs use the same element.
...
This produces an even more filtered graph $\mathit{grd}$, which only contains arguments that are both alive and in the grounded extension.

From this graph $\mathit{grd}$, we want to produce a reward as a scalar number: this is the goal of the judgment function $\Jj$.
...
$\Jj$ intuitively counts the number of arguments in $\mathit{grd}_{[\Fp]}$, and the number of arguments in $\mathit{grd}_{[\Fc]}$ and compares them.
...
If the agent's behaviour improves, in the human's eye, but the reward does not increase, the agent will probably try another behaviour.
On the other hand, if the reward effectively increases, this will signal the learning algorithm that it should continue in this direction.

For example, if we suppose the number of cons arguments stayed the same, but the number of pros arguments increased, the resulting reward should increase as well.
...
\Fn{\FnJudgment{}}{
  ...
  \ForEach{learning agent $l \in \mathbb{L}$}{
    ...
    \ForEach{judging agent $j \in \mathbb{J}$}{
      ...
      $\mathit{grd} \gets \texttt{grounded}(AF')$\;
      \tcc{Compute reward through the $\Fp$ and $\Fc$}
      $\texttt{F}(l)_j \gets \Jj(\mathit{grd})$\;
    }
    \tcc{Agregate judgments into a single reward}
    $r_l \gets \mathtt{g_{agr}}\left( \left\{ \forall f_{l,j} \in \texttt{F}(l) \right\} \right)$\;
  }
  \Return{$\mathbf{r}$}
}
```
...
#|   filtered based on the current situation and taken action to remove arguments
...
These experiments are based on the moral values we previously described: security of supply, inclusiveness, environmental sustainability, and affordability.
...
It might seem curious to remove moral values ; we use this scenario as proof that, in combination with the ability to add moral values, we can effectively learn to change the ethical consensus within the society.
...
::: {.remark}
Note that, as the goal of this contribution is to propose an alternative way of specifying the reward function, i.e., through symbolic judgment rather than mathematical functions, the reward functions in these experiments are different from those used in the first contribution's experiments.
...
Indeed, the goal is not here to evaluate whether the algorithm achieves the best score, but rather to evaluate whether:
...
In both logic-based and argumentation-based models, we implement judging agents that represent the moral values detailed in Section \@ref(positioning-smartgrid-moral):
...
* Environmental Sustainability, which forbids transactions with the national grid.
...
* The consumption profile, i.e., *annually* or *daily*.
...
The *annually* profile contains a consumption need for every hour of every day in a year, which we use as a target for agents: they want to consume as much as the need indicates ; the *daily* profile is averaged on a single day of the year, and therefore does not contain the seasonal variations.
...
The CArtAgo environment, defined in the Java programming language, allow us to represent the simulation and learning algorithms by storing the received observations and actions, and making them available to the judging agents.
...
* The configuration of judges, i.e., which judge is activated and when.
...
<!-- TODO: show an example of argumentation graph, and put the others in an appendix -->
...
From this grounded extension $\mathit{grd}$ and the sets $\Fp$, $\Fc$, the $\Jj$ function must output a reward, i.e., a single number, that corresponds to which degree the agent respected the moral value.
...
Indeed, the moral valuations are given by the $\Fp$ and the $\Fc$, and any argument can be either a pro or con argument.
...
afdm
: Function that simply compares the number of $\Fp$ with the sum of $\Fp$ and $\Fc$ in $\mathit{grd}$. This function is simple and intuitive to understand, however, it fails to take into account difference when there are no $\Fp$ or $\Fc$. To illustrate this, let us assume that, at a given step $t$, there were 3 arguments in $\Fp \cap \mathit{grd}$, and 0 in $\Fc \cap \mathit{grd}$. The result is thus $\frac{3}{3} = 1$. This correctly represents the fact that, at $t$, the action only had moral evaluations (for this specific moral value). Let us also assume that, at another step $t'$, there were still 0 arguments in $\Fc \cap \mathit{grd}$, but this time 4 arguments $\Fp \cap \mathit{grd}$. One would intuitively believe that the action at $t'$ was better than the one at $t$: it received more moral evaluations. However, this function returns in this case $\frac{4}{4} = 1$, which is the same reward. There is virtually no difference between an action with 3 or 4 pros arguments, as long as there is 0 cons arguments. The exact same problem applies with 0 arguments in $\Fp$, which would invariably return $\frac{0}{x} = 0$.
: $$\texttt{J}_{afdm} = \begin{cases}
  \frac{\length{\FpInGrd}}{\length{\FpInGrd} + \length{\FcInGrd}} & \text{if } \length{\FpInGrd} + \length{\FcInGrd} \neq 0 \\
  \frac{1}{2} & \text{else}
\end{cases}$$

leturc
: Function that compares first the number of $\Fp$ in the grounded with the total number of $\Fp$ in the unfiltered, original, argumentation graph, then does the same for the number of $\Fc$, and finally compares the $\Fp$ and the $\Fc$. Thus, this function is able to take into account the existence of non-activated arguments, either pros or cons, when creating the reward. For example, if only 3 of the 4 possible $\Fp$ arguments are activated, the function will not return $1$. Intuitively, this can be thought as "You have done well... But you could have done better." In this sense, this function seems better than the previous one. However, a problem arises if some arguments are too hard to activate, and therefore (almost) never present in the $\mathit{grd}$, or if 2 arguments cannot be activated at the same time. The agent will thus never get $1$, or conversely $0$ in the case of $\Fc$, because the function relies on the assumption that all arguments can be activated at the same time.
: $$\texttt{J}_{leturc} = \frac{\length{\FpInGrd}}{\length{\FpTotal}} - \frac{\length{\FcInGrd}}{\length{\FcTotal}}$$

leturc2
: Function that compares the sum of the number of $\Fp$ and $\Fc$ with the maximum known of activated $\Fp$ and $\Fc$ at the same time. This function thus avoids the pitfalls of the previous one, by only comparing with a number of activated arguments that is realistically attainable. However, it still suffers some drawbacks. For example, we cannot compute *a priori* the maximum number of activated arguments ; instead, we maintain and update a global counter. This means that the rewards' semantics may change over the time steps: a $\frac{1}{2}$ might be later judged as a $\frac{1}{3}$, simply because we discovered a new maximum. Taking this reasoning further, we can imagine that maybe, in a given simulation, the maximum known number will be 2 for most of the time steps, and finally during the later steps we will discover that in fact the maximum could attain 4. Thus, all previous rewards would have deserved to be $\frac{x}{4}$ instead of $\frac{x}{2}$, but we did not know that beforehand.
: \begin{align*}
  \mathit{top} & = \length{\FpInGrd}^2 - \length{\FcInGrd}^2 \\
  \mathit{down} & = \length{\FpInGrd} + \length{\FcInGrd} \\
  \mathit{max\_count} & = \max( \mathit{max\_count}, \mathit{down} ) \\
  \texttt{J}_{leturc2} & = \frac{\mathit{top}}{\mathit{max\_count}}
\end{align*}

remy
: Function that takes into account the total possible number of $\Fp$ and $\Fc$ arguments. This function creates a gradient between $0$ and $1$, with as many graduations between $0.5$ and $1$ as there are possible pros arguments, and conversely as many graduations between $0.5$ and $0$ as there are possible cons arguments. Each activated pro argument advances one graduation towards $1$, whereas each activated con argument advances one graduation towards $0$, with a base start of $0.5$. This function is similar to leturc, except that it stays within the $[0,1]$ range.
: $$\texttt{J}_{remy} = 0.5 + \length{\FpInGrd} \times \left( \frac{0.5}{\length{\FpTotal}} \right) - \length{\FcInGrd} \times \left( \frac{0.5}{\length{\FcTotal}} \right)$$

benoit
: Function that avoids division by $0$ by simply offsetting the number of activated arguments. Also, since neither the numerator nor the denominator can be $0$, adding one $\Fc$ argument without changing the number of $\Fp$ would effectively change the resulting reward. For example, assuming $\length{\Fp} = 0$, and $\length{\Fc} = 3$, the reward would be $\frac{1}{4}$. Adding one $\Fc$ argument would yield $\frac{1}{5}$, thus effectively reducing the reward. However, to avoid having rewards superior to $1$, the use of the $\min$ function prevents the same effect on $\Fp$: if $\length{\Fc} = 0$, the reward will be $1$ no matter how many $\Fp$ are present.
: $$\texttt{J}_{benoit} = \min \left( 1, \frac{1 + \length{\FpInGrd}}{1 + \length{\FcTotal}} \right)$$


#### The aggregation function

As for the logic-based judgment, we have several judges that each provide a reward for every learning agent.
...
Benoît proposed to compare it with a *min* aggregation that is based on the Aristotle principle.
...
$$
  \mathtt{g_{min}}\left(\texttt{F}(l)\right) = \min \left( \left\{ \forall f_{l,j} \in \texttt{F}(l) \right\} \right)
  (\#eq:aggregation-min)
$$

where $\texttt{F}(l)$ is the set of rewards for the learning agent $l$, of size $\length{\JAgts}$, and $f_{l,j}$ is the specific reward for learning agent $l$ determined by judging agent $j$.

However, I noticed that in some cases this function could become "stuck".
For example, if one the argumentation graph is ill-defined, and it is impossible to get a high reward for one of the moral values, e.g., more than $0.2$.
...
It is not a problem in the case of a few steps, because the agent will have later steps to learn other moral values.
...
$$
  \mathtt{g_{\textsc{WR}}}\left(\texttt{F}(l)\right) = \text{draw } f_{l,X} \sim P(X = j) = \frac{1 - f_{l,j}}{\displaystyle\sum_{k \in \JAgts} \left(1 - f_{l,k}\right)}
...
Finally, we propose, similarly to the logic-based judges, several configuration of argumentation judges.
...
Some values seem easier to learn, e.g., the *affordability* and *supply_security* scenarii, which consistently have a score of nearly $1$.
Others, such as the *inclusiveness*, seem harder to learn, but attain a score of $0.5$ nevertheless.

```{python judgments-results-logic-scores, out.width = "100%"}
#| fig.cap: >
#|   Results of the learning algorithms on 10 runs for each scenario, when using the logic-based judging agents.
import pandas as pd
...
The *benoit*, *remy*, and *afdm* seem indeed to yield higher rewards.
...
```{python judgments-results-argumentation-scores, out.width = "100%"}
#| fig.cap: >
#|   Results of the learning algorithms on 10 runs for each scenario, when
#|   using the argumentation-based judging agents. To simplify the plot, the
#|   EnvironmentSize and ConsumptionProfile variables were combined into a
#|   single *Scenario* variable. Similarly, the *Functions* variable combines
#|   the choice of the judgment and aggregation functions.
import pandas as pd
import matplotlib.pyplot as plt
import seaborn as sns
sns.set()
...
df['Scenario'] = df['EnvironmentSize'] + ' / ' + df['ConsumptionProfile']
df['Functions'] = df['JTransform'] + ' / ' + df['Aggregation']
g = sns.catplot(data=df,
                # kind='violin',
                x='Judges',
                y='Score',
...
In this chapter, we have presented a new line of research for constructing reward functions, through the use of symbolic judgment.
More specifically, we propose to introduce new judging agents to the multi-agent system, which judge the behaviour of the learning agents to determine their rewards, by leveraging symbolic reasoning, through either logic-based rules or argumentation frameworks.
To the best of our knowledge, this line of research has not been studied, especially within the Machine Ethics.
...
We have given the fictional example of an agent which learned to hoard then give energy, to demonstrate how this kind of reward gaming could be fixed, by adding a new argument that checks whether the agent hoarded energy at the previous step.
...
The proposed methods in this chapter, logic-based and argumentation-based, have various specificities, and we now compare them, both to the traditional mathematical reward functions, and between them.
...
As we have seen in the state of the art, multiple works have tried to implement ethical principles or moral values as symbolic rules.
...
However, we note that one disadvantage of such symbolic rules, when applied to the problem of generating rewards for learning agents, is to offer a poorer gradient than mathematical formulas, as we have already mentioned.
...
It therefore has no feedback indicating that its action was better, still not sufficient, but that it could continue in this direction.
...
This problem of gradient is linked with the notion of *moral evaluation* that we have proposed in our methods.
...
We note a first difference, which is purely an implementation detail, between our two methods: in LAJIMA, we have chosen to produce exactly as much evaluation as there are dimensions in an action ; thus, each action dimension is judged and represented by a moral evaluation.
On the contrary, in AJAR, we can have as many pros and cons arguments, and thus moral evaluations.
...
It also opens the way for different methods to produce rewards: we have proposed a few non-exhaustive options in our experiments to illustrate this.
This may be seen as a disadvantage, since it requires an additional parameter of selecting the judgment function, which implies fine-tuning.
Yet, this disadvantage may be justified, as the experiments tend to show a better gradient for the AJAR framework.
...
Indeed, we have used a simple argumentation framework in our proof-of-concept work: the attack relationship is a simple, unidirectional one.
...
Intuitively, we can see this kind of judgment as "Would I have done the same thing, have I been at this agent's place?".
...
Some of them probably will not want to share data.
...
For example, the well-known Mycin example has shown that expert systems, which rely on similar rules to our LAJIMA model, were difficult to understand by doctors, although it could theoretically "explain" its reasoning by showing the trace of rules that were triggered.
...
Both our models have a small limitation to simplify their definition.
...
However, one might argue that moral values are not the only important objectives: there might be some other, that we denote "technical aims", or aims for short, which are important for the system but not especially linked to a moral value.
An example of such aim may be, for example, to correctly balance the production and consumption within a Smart Grid.
...
However, if they would be easier to design as a mathematical function, as the example above, which can be simply formulated as a difference between the production and the consumption, the current model does not allow this.
We note that we can extend the formal definitions of the model to consider not only the set of judging agents, but rather the union of judges with aims functions.
