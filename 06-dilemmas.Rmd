...
* State-(D)SOM: the "map" of situations to discrete states, which contains a fixed number of neurons, and thus of possible states. We define the set of possible states identifiers as $\mathcal{S} = [[0, \cdots, \length{\mathbf{U}}]]$, where $\length{\mathbf{U}}$ is the number of neurons.
...
* Action-(D)SOM: the "map" of action identifiers to action parameters, which also contains a fixed number of neurons, and thus of possible actions. The set of possible action identifiers is $\mathcal{A} = [[0, \cdots, \length{\mathbf{W}}]]$, where $\length{\mathbf{W}}$ is the number of neurons. Each neuron is associated to a prototype vector, which is the action's parameters.
...
* $\rho$ : the weights used to scalarize rewards and interests to determine whether a perturbed action is interesting, a vector $\in \mathbb{R}^m$.
:::
...
Note that, in addition, the Bellman equation must be adapted as well:
...
For example, considering an action $a_1$ with interests $\texttt{Q}(s, a_1) = [0.5, 0.5]$ and another action $a_2$ with interests $\texttt{Q}(s, a_2) = [1, 0]$, we can say that taking action $a_1$ would result in a regret with respect to the first moral value, as $a_2$ would have yielded a better result.
...
\begin{equation}
  \PFFn(\mathbf{o}) = \left\{
     \forall (p, a) \in (\Profiles, \DiscreteActions) : (p, a) | \nexists (p', a') \in (\Profiles, \DiscreteActions) \Q_{p'}(\StateFn_{p'}(\mathbf{o}), a') \gPareto \Q_p(\StateFn_p(\mathbf{o}), a) \right\}
  (\#eq:pareto-front)
\end{equation}
...
Learning agents receive observations, and select actions based on the number of times they were already chosen, and noise the action parameters to explore the action space.
The perturbed action is deemed interesting or not based on the exploration profile's weights and Equation \@ref(eq:interesting-proposed-action-morl).
Q-Values are updated using Equation \@ref(eq:bellman-morl), and, similarly, Q-theoretical values are updated thanks to Equation \@ref(eq:theoretical-bellman-update).

```{algorithm dilemmas-bootstrap-algorithm, fig.cap = "Learning process during the bootstrap phase", fig.pos = "!h"}
...
To select an action, agents compute the Pareto Front (PF) of optimal actions, as described in Equation \@ref(eq:pareto-front) and then determine whether each of them is an acceptable action, according to Definition \@ref(def:acceptable-action), leveraging the ethical thresholds set by the human user.
If no acceptable action can be found, the situation is a dilemma, and the agent looks for a context that would recognize this dilemma, as defined in Definition \@ref(def:dilemmas-context).
When such a context is found, the action that was associated to this context by the user is selected; otherwise, an interface asks the user for which action to be enacted, and the definition of a context in which the same action should be selected again.
