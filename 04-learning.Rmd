...
Smith's initial work proposed, in order to handle continuous domains, to associate Self-Organizing Maps (SOMs) to a Q-Table.
...
In response, the environment changes its state, and sends them new observations, potentially different for each agent, corresponding to this new state, as well as a reward indicating how correct the performed action was.
...
The learning process updates these same structures, and takes the reward as an input, in addition to observations.
...
The learning rate $\alpha$, and the neighborhood function $\theta$ both depend on the time step $t$: they are often monotonically decreasing, in order to force neurons' convergence and stability.
...
Whereas SOMs and other similar algorithms try to learn the density of data, DSOMs focus on the structure of the data space, and the map will not try to place several neurons in a high-density region.
...
$\left\langle \LAgts, \StateSpace, \ActionSpace, \texttt{T}, \ObsSpace, \texttt{O}, \RewardFn, \gamma \right\rangle$,
where:
...
Our algorithms are separated into 2 distinct parts: the *decision* process, which chooses an action from received observations about the environment, and the *learning* process, which updates the algorithms' data structures, so that the next decision step will yield a better action.
...
\Fn{\FnDecision{}}{
  \KwData{\\
    ...
    $\quad \Q(s,a)$ the Q-value of action $a$ in state $s$\\
...
First, we need to obtain a discrete identifier from an observation $\mathbf{o}$ that is a vector $\in \ObsSpace_l \subseteq \RR^{g}$, in order to access the Q-Table.
...
\frac{
  \frac{\exp(\Q(s,j))}{\tau}
  }{\sum_{k = 1}^{\length{\mathcal{W}}} \frac{\exp(\Q(s,k))}{\tau}}
...
\Fn{\FnLearning{}}{
  \KwData{\\
    ...
    $\quad \Q(s,a)$ the Q-value of action $a$ in state $s$\\
    $\quad \eta_{U}, \eta_{W}$ elasticity for State- and Action-(D)SOMs\\
    $\quad \alpha_{Q}, \alpha_{U}, \alpha_{W}$ learning rates for Q-Table, State-, Action-(D)SOMs\\
    $\quad \gamma$ the discount factor\\
  }
  \KwIn{\\
...
This may be difficult as the number of agents increases, since there will most certainly be more conflicts.
...
* Then, the 4 algorithms, configured with their best hyperparameters, were compared on multiple settings:
  ...
  This resulted in $2 \times 2 \times 7$ scenarii, which we ran $10$ times for each of the $4$ algorithms.
...
```{python learning1-results-compare, out.width = "100%", fig.pos = "!ht"}
#| fig.cap: >
...
*Daily* scenarii are perhaps too easy for the agents as they do not include as many variations as the *annual*; additionally, *small* scenarios are easier to visualize and explore, as they contain fewer agents than *medium* scenarios.
