...
They are based on an existing work [@smith2002applications; @smith2002dynamic] that we extend and evaluate in a more complex use-case.
Our 2 proposed algorithms are meant as a first step, a building basis on top of which we will add improvements and features throughout our next contributions.
...
As per our hypothesis, and as motivated in our state of the art of Machine Ethics, we focused on continuous multi-dimensional domains, for both states and actions.
Smith's initial work proposed to leverage Self-Organizing Maps (SOMs) to handle these continuous domains.
...
It includes multiple learning agents that perform actions in a shared environment. In response, the environment changes its state, and sends them observations, potentially different for each agent, corresponding to this new state, as well as a reward indicating how correct the performed action was.
...
The unique index of the BMU is used as the state identifier $s$.
...
Knowing these interests $\Q(s,.)$ for all actions, we can assign a probability of taking each one, using a Boltzmann distribution.
...
We draw an action identifier $j$ from the list of possible actions, according to Boltzmann probabilities.
...
#|   *annual small* scenario and *adaptability2* reward function.
...
#| tab.cap: >
#|   Best hyperparameters on 10 runs for the MADDPG algorithm, using the
#|   *annual small* scenario and *adaptability2* reward function.
